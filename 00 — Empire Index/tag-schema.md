...
| Cross-link | `#applies-to/website-dev` `#applies-to/ai-agents` `#applies-to/ai-agents/chat` `#applies-to/ai-agents/voice` `#applies-to/ai-agents/marketing` `#applies-to/craft/copywriting` `#applies-to/craft/psychology` `#applies-to/craft/sales` `#applies-to/craft/design` `#applies-to/optimus-inc/finance` `#applies-to/optimus-inc/marketing` `#applies-to/optimus-inc/operations` `#applies-to/optimus-inc/brand` `#applies-to/tools/<tool-slug>` `#applies-to/anthony-rosa/investments` `#applies-to/anthony-rosa/projects` `#applies-to/anthony-rosa/skills` | Used on bridge notes (`apply-to-optimus/` and `apply-to-anthony-rosa/`) to surface where the captured knowledge integrates. Six target zones: offerings (`website-dev`, `ai-agents/{chat,voice,marketing}`), cross-cutting craft (`craft/{copywriting,psychology,sales,design}`), Optimus the company itself (`optimus-inc/{finance,marketing,operations,brand}`), tool evaluations (`tools/<tool-slug>`), the existing offering hubs, and the Anthony Rosa personal hub (`anthony-rosa/{investments,projects,skills}`). The `#applies-to/optimus-inc/marketing` tag is distinct from the Marketing-Team OFFERING — Optimus-Inc/marketing is the company's own marketing voice, the offering is the productized Marketing-Team agent. The `#applies-to/anthony-rosa/skills` tag means career skill progression (e.g., Python, LangChain) — distinct from Claude Code skills at `.claude/skills/` and Optimus build skill instructions. |
...
| Owner | `#owner/optimus` `#owner/anthony-rosa` | Domain ownership: Optimus the LLC (productized services for SMB clients) vs. Anthony Rosa (personal layer — career, investments, personal automated revenue projects, skill goals). Notes that span both domains carry BOTH tags. Bridge files in `apply-to-optimus/` carry `#owner/optimus`; bridge files in `apply-to-anthony-rosa/` carry `#owner/anthony-rosa` — never both on a single bridge file (cross-domain skills get TWO single-zone bridge files instead). |
...
- 2026-05-08 — Two-domain vault restructure: added new tag family `#owner/{optimus,anthony-rosa}` for domain ownership distinction (Optimus LLC vs. Anthony Rosa personal layer). Schema now contains 8 tag families total. The `anthony-rosa/` folder was simultaneously elevated to a 5th peer hub alongside the four Optimus hubs; a 6th `/learn` bridge zone (`apply-to-anthony-rosa/`) was added with sub-targets `#applies-to/anthony-rosa/{investments,projects,skills}` (these live within the existing `#applies-to/*` family rather than expanding it as a separate change). See `optimus-system-guide.md` Section 3 and `anthony-rosa-peer-hub-elevation.md` plan for full context.

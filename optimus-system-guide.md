...
The four AI agent products share one Python stack from day one: FastAPI · anthropic SDK (Claude API direct) · Pydantic v2 · supabase-py · Twilio (SMS + voice telephony) · Personaplex (voice model). Tier-4 only adds an open-source agent harness and private per-client GPU compute. The canonical reference lives at `[[Offerings/02 AI Agents/shared-knowledge/tech-stack]]` — no visual workflow orchestrators or third-party voice platforms.
...
### Orchestration discipline (added 2026-05-02)

Five orchestration rules tightened in [[CLAUDE]] govern how every website-dev session runs: a Pre-Task Memory Search Protocol that consults MEMORY.md and `knowledge/build-log.md` before each agent spawn, a Concurrency Checkpoint that mandates parallel agent spawning in a single message when 3+ tasks are independent, a Post-Task Capture Ritual that captures surprises at the moment of insight, an explicit "never stop after agent spawn" rule, and a clean ledger-vs-executor boundary. Model selection per agent is governed by [[agent-routing]] at vault root — `effort:` frontmatter maps to specific Claude model IDs there, never ad-hoc.
...
- 2026-04-29 — Added `anthony-rosa/` founder layer at vault root with The End Goal autonomous-AI-employees vision, Drink-Own-Champagne milestone (2027-Q3), reinvestment loop, moat statement. Established canonical Python stack (FastAPI + anthropic SDK + Pydantic + supabase-py + Twilio + Personaplex) for all upsells — codified in `Offerings/02 AI Agents/shared-knowledge/tech-stack.md`. No visual workflow orchestrators in the stack. Added Tier-4 Autonomous AI Employee as fourth product line in `Offerings/02 AI Agents/04 Autonomous Employee/` — custom-trained per client on open-source harness, private per-client GPU deployment, $7,500-15,000 setup + $2,500-5,000+/mo. Added shared `agent-infrastructure.md` establishing four primitives (memory · tools · observability · approval) used across Tiers 2/3/4. Commit <pending>.
- 2026-05-02 — Tightened orchestration discipline in [[CLAUDE]] with five new/edited rules extracted from Ruflo patterns (Pre-Task Memory Search Protocol, Concurrency Checkpoint, Post-Task Capture Ritual, NEVER-stop rule, ledger-vs-executor boundary). Added [[agent-routing]] at vault root mapping agent `effort:` frontmatter to canonical Claude model IDs. Zero infrastructure additions, zero agent file changes — the seven existing agents are unchanged; only the orchestrator's operating habits sharpen. Commit <pending>.
